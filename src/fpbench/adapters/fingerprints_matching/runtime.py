"""The artifact and the runtime this algorithm will actually execute on.

Lives in the adapter package rather than beside the stage that publishes it,
because the adapter has to verify this closure before it reports itself ready and
an adapter may import ``fpbench.core`` and itself and nothing else of fpbench.
Stage 15A's G1 is this module's output, read from
:mod:`fpbench.experiments.stage15a_runtime`.
...
STORE_RELATIVE = frozen.STORE_RELATIVE


def file_sha256(path: Path) -> str:
    """A digest of one file, read in blocks.

    Duplicated from :mod:`fpbench.third_party.artifacts` rather than imported,
    because that package sits above this layer. Twelve lines of hashlib is a
    cheaper price than an adapter that reaches upward.
    """
    ...
    try:
        ...
        raise Stage15ARuntimeIdentityError(f"cannot hash {Path(path).name}: {exc}") from exc
    return digest.hexdigest()


def resolve_store_root(*, repository_root: Path | None = None) -> Path:
    """The third-party store root for this machine, checked before it is returned.

    Same two-step resolution ``fpbench.third_party.artifacts`` performs — the
    environment variable, then the default under the user's home — and the same
    refusal of a root inside the working tree, because a store there is one
    ``git add -A`` away from publishing vendor bytes (docs/adr/0083).
    """
    raw = str(os.environ.get(frozen.THIRD_PARTY_ROOT_ENV, "")).strip()
    if raw:
        root = Path(raw)
        if not root.is_absolute():
            raise Stage15ARuntimeIdentityError(
                f"{frozen.THIRD_PARTY_ROOT_ENV}={raw!r} is not an absolute path"
            )
    else:
        root = Path.home().joinpath(*frozen.DEFAULT_STORE_RELATIVE)
    if repository_root is not None:
        ...
        if (
            ...
            raise Stage15ARuntimeIdentityError(
                ...
                "repository. Third-party bytes live outside the working tree "
                "(docs/adr/0083)"
            )
    return root
...
def store_root(*, repository_root: Path | None = None) -> Path:
    return resolve_store_root(repository_root=repository_root) / STORE_RELATIVE
