...
RESEARCH_RECEIPT_SCHEMA_VERSION = "3"
RESEARCH_FINALIZATION_SCHEMA_VERSION = "4"
...
def _freeze_digests(value: Mapping[str, str], field_name: str) -> Mapping[str, str]:
    ...
    for key, digest in dict(value).items():
        role = str(key)
        validate_id(role)
        digests[role] = _require_digest(digest, f"{field_name}[{role}]")
    if not digests:
        raise ValueError(f"{field_name} must name at least one runtime asset")
...
@dataclass(frozen=True, slots=True)
class LegacyResearchRunReceipt:
    ...
    This type preserves receipt schemas 1 and 2 exactly as published. New runs
    use :class:`ResearchRunReceipt`; keeping the old field names here is an
    explicit compatibility boundary, not part of the generic contract.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version not in {"1", "2"}:
            raise ValueError(
                f"unsupported legacy research receipt schema {self.schema_version!r}"
            )
...
class ResearchRunReceipt:
    """Algorithm-neutral receipt emitted for integration-bound research runs."""
    ...
    runtime_bundle_id: str
    runtime_bundle_fingerprint: str
    runtime_asset_sha256s: Mapping[str, str]
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            _freeze_digests(self.runtime_asset_sha256s, "runtime_asset_sha256s"),
        )
        ...
        if (
            ...
            raise ValueError("success and failure counts must add up to stored results")
        ...
        if version != RESEARCH_RECEIPT_SCHEMA_VERSION:
            raise ValueError(f"unsupported research receipt schema {version!r}")
...
ResearchReceipt = LegacyResearchRunReceipt | ResearchRunReceipt


def require_sanitised(receipt: "ResearchReceipt") -> None:
...
def research_receipt_fingerprint(receipt: ResearchReceipt) -> str:
...
def research_receipt_content_hash(receipt: ResearchReceipt) -> str:
...
def _research_receipt_plain(receipt: ResearchReceipt) -> dict[str, Any]:
    """Render a receipt without retroactively changing schema-v1 identities."""
    plain = dict(to_plain(receipt))
    if isinstance(receipt, LegacyResearchRunReceipt) and receipt.schema_version == "1":
        for name in (
            "preparation_set_id",
            "preparation_set_fingerprint",
...
@dataclass(frozen=True, slots=True)
class LegacyResearchFinalizationMarker:
...
class ResearchFinalizationMarker:
    """Algorithm-neutral final authority for integration-bound research runs."""
    ...
    def __post_init__(self) -> None:
        for name in ("finalization_id", "run_id", "plan_id", "integration_id"):
            validate_id(str(getattr(self, name)))
        for name in (
            "finalization_fingerprint",
            "run_fingerprint",
        ...
        if version != RESEARCH_FINALIZATION_SCHEMA_VERSION:
            raise ValueError(
                f"unsupported research finalization schema version {version!r}"
            )
        object.__setattr__(self, "schema_version", version)
        created = str(self.created_utc).strip()
        if not created:
...
ResearchFinalization = LegacyResearchFinalizationMarker | ResearchFinalizationMarker


def research_finalization_fingerprint(
    marker: ResearchFinalization | Mapping[str, Any],
) -> str:
