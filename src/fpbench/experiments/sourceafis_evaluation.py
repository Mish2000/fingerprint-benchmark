"""Counting a finished SourceAFIS derivation, once.

The sibling of :mod:`fpbench.experiments.sourceafis_decisions`, one layer up and
for the same reason. Stage 5B wrote this logic for the native derivation; stage
6B needs the identical logic for the canonical one, and copying the file would
mean a difference between the two sets of counts could be a difference in how
they were counted.
...
copies go. Everything else is shared.

**It applies no threshold and tries no other one.** Threshold 40 came from
SourceAFIS's own documentation, was fixed in stage 5A, and is transferred
unchanged to the canonical inputs in stage 6B. There is no code path in this
stage that could change it, and that is not an oversight (docs/adr/0021).
"""
...
@dataclass(frozen=True, slots=True)
class SourceAfisEvaluationExperimentSpec:
    """The three things that distinguish one SourceAFIS evaluation from another."""

    evaluation_config: Path
    decision_spec: SourceAfisDecisionExperimentSpec
