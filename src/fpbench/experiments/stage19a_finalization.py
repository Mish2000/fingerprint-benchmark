...
def build_stage19a_finalization(
    *,
    ...
    # A false machine condition decides the conjunction whatever the human has
    # or has not determined: ``null`` means "nobody has ruled on the last
    # condition", not "the ones that were ruled on can be ignored".
    failed = sorted(name for name, value in conditions.items() if value is False)
    if failed:
        established: bool | None = False
    elif sufficiency == "UNDETERMINED":
        established = None
    else:
        established = all(bool(value) for value in conditions.values())

    outcome = frozen.OUTCOME_NOT_COMPLETE if failed else frozen.OUTCOME_COMPLETE
