...
def test_failure_after_each_intermediate_write_is_retryable_and_not_ready(
    ...
    monkeypatch.setattr(experiment, "_load_prepared", lambda **_: prepared)
    monkeypatch.setattr(
        experiment,
        "validate_sourceafis_result_set",
        lambda **_: structural_validation_report(world),
    )
    monkeypatch.setattr(experiment, "write_evidence_copy", lambda *_, **__: None)
    ...
    if write_step == "result_set":
        ...
        monkeypatch.setattr(
            ...
            fail_once_after(experiment.write_operational_summary),
        )
    else:
        monkeypatch.setattr(
            prepared.result_store,
